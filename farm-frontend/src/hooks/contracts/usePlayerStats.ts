...
export function usePlayerStats() {
  ...
  // 获取玩家排名 - 暂时使用模拟数据，因为合约可能没有这些函数
  const harvestRank = 0 // TODO: 从合约获取
  const kindnessRank = 0 // TODO: 从合约获取
  const harvestRankLoading = false
  const kindnessRankLoading = false
  ...
  return {
    ...
    // 加载状态
    isLoading: statsLoading || harvestRankLoading || kindnessRankLoading || todayHelpLoading || playerInfoLoading,
    error: statsError || todayHelpError,
